...
const ProfilePage = () => {
  ...
  const BACKEND_URL = "http://app:8080/api/users";
  // const ORDER_HISTORY_URL = `http://localhost:8080http://app:8080/api/foods/user/${userId}`;
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`http://app:8080/api/orders/user/${userId}`);
