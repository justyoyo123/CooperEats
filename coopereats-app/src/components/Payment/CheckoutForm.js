...
const CheckoutForm = () => {
    ...
    const loadSavedPaymentMethods = async () => {
        ...
        try {
            ...
            const { data: paymentInfo } = await axios.get(`http://app:8080/api/paymentinfo/${userId}`);
    ...
    const handleSubmit = async (event) => {
        ...
        const cartResponse = await axios.get(`http://app:8080/api/carts/user/${userId}`);
        ...
        for (const [foodId, quantityOrdered] of Object.entries(cartItems)) {
            try {
                const foodResponse = await axios.get(`http://app:8080/api/foods/${foodId}`);
        ...
        try {
            ...
            const { data: paymentIntentResponse } = await axios.post('http://app:8080/api/payments/charge', paymentPayload);
            ...
            if (confirmResult.error) {
                ...
                const orderResponse = await axios.post('http://app:8080/api/orders/placeOrder', orderRequest);
    ...
    const handleFoodUpdate = async () => {
        try {
            const orderResponse = await axios.get(`http://app:8080/api/orders/${orderId}`);
            ...
            for (const [foodId, orderedQuantity] of Object.entries(orderedProducts)) {
                ...
                await axios.post(`http://app:8080/api/foods/modifyQuantity/${foodId}`, { quantity: newQuantity });
