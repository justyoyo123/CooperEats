...
const AdminPage = () => {
    ...
    const fetchFoods = async () => {
        try {
            const response = await axios.get('http://app:8080/api/foods');
    ...
    const handleIncreaseQuantity = async (foodId) => {
        try {
            ...
            await axios.post(`http://app:8080/api/foods/modifyQuantity/${foodId}`, { quantity: new_quantity });
    ...
    const handleDeleteItem = async (foodId) => {
        try {
            const response = await axios.delete(`http://app:8080/api/foods/${foodId}`);
